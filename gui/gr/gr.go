...
func DrawCircle(pixmap Pixmap, bounds Rect, x0, y0, radius int, clr color.Color) {
	...
	x0 = ints.Max(x0, bounds.X0)
	x1 = ints.Min(x1, bounds.X1-1)

	y0 = ints.Max(y0, bounds.Y0)
	y1 = ints.Min(y1, bounds.Y1-1)
...
func DrawLine(pixmap Pixmap, bounds Rect, x0, y0, x1, y1 int, clr color.Color) {
	if x0 == x1 {
		...
		var tmp Rect
		tmp.X0 = ints.Min(x0, x1)
		tmp.Y0 = ints.Min(y0, y1)
		tmp.X1 = ints.Max(x0, x1) + 1
		tmp.Y1 = ints.Max(y0, y1) + 1
